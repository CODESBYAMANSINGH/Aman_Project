...
// Variants for the sun's core style (gradient and intense inner glow)
const sunCoreVariants = {
  day: {
    background:
      "radial-gradient(circle, #FFFFFF 20%, #FFD700 60%, #FFA500 100%)",
    boxShadow:
      "0 0 35px 10px #fff, 0 0 60px 20px #ffc72c, 0 0 110px 30px #f09819",
  },
  evening: {
    background:
      "radial-gradient(circle, #FFEDA0 20%, #FF8C00 70%, #D2691E 100%)",
    boxShadow:
      "0 0 35px 10px #ffccbc, 0 0 60px 20px #ff9f80, 0 0 110px 30px #ef6c00",
  },
};

// Variants for the moon image (handles opacity)
const moonVariants = {
  day: { opacity: 0 },
  evening: { opacity: 0 },
  night: {
    ...
    <div className="fixed top-16 right-16 w-32 h-32 z-0">
      ...
      >
        ...
        >
          {/* Corona Effect */}
          <motion.div
            className="absolute -inset-4 w-[125%] h-[125%] rounded-full"
            style={{
              background: "radial-gradient(circle, rgba(255, 223, 186, 0.5) 60%, transparent 75%)",
              filter: "blur(8px)",
            }}
            animate={{ rotate: -180 }}
